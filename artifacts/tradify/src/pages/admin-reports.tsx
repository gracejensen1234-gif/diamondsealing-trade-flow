...
function isActivePhoto(photo: unknown): photo is string {
  return typeof photo === "string" && photo.startsWith("data:image");
}

function isDeletedPlaceholder(
  photo: unknown,
): photo is RetainedPhotoPlaceholder {
  return (
    typeof photo === "object" &&
    photo !== null &&
    (photo as RetainedPhotoPlaceholder).status ===
      "deleted_under_retention_policy"
  );
}

function retentionBadgeClass(status?: string) {
  if (status === "deleted_under_retention_policy") {
...
export default function AdminReports() {
  ...
  const totalMetres =
    reports?.reduce((sum, r) => sum + r.metersCompleted, 0) || 0;
  const issuesCount =
    reports?.filter((r) => r.issueType !== "none").length || 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Review completed jobs and issues from the field.
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          reports?.map((report) => {
            const photos = Array.isArray(report.photos)
              ? (report.photos as unknown[])
              : [];
            const activePhotos = photos.filter(isActivePhoto);
            const deletedPhotos = photos.filter(isDeletedPlaceholder);
            const photoRetention = (
              report as { photoRetention?: PhotoRetentionSummary }
            ).photoRetention;
                    ...
                    <CardDescription className="flex flex-wrap gap-2 mt-1">
                      ...
                      {photoRetention ? (
                        <>
                          ...
                          <Badge
                            variant="outline"
                            className={retentionBadgeClass(
                              photoRetention.status,
                            )}
                          >
                            Photos: {photoRetention.label}
                          </Badge>
                        </>
                      ) : null}
                    </CardDescription>
                      ...
                      <div>
                        ...
                        {report.silikoneColoursUsed.length > 0 ? (
                          <div className="flex gap-1 mt-1 flex-wrap">
                            {report.silikoneColoursUsed.map((c) => (
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button variant="outline" size="sm">
                          ...
                          {activePhotos.length}/{photos.length})
                        </Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-3xl">
                        <DialogHeader>
                          <DialogTitle>
                            Photos for {report.jobTitle}
                          </DialogTitle>
                        </DialogHeader>
                        <div className="grid grid-cols-2 gap-4 py-4 max-h-[70vh] overflow-y-auto">
                          {activePhotos.map((url, idx) => (
                            <div
                              key={idx}
                              className="aspect-video relative rounded-md overflow-hidden bg-muted"
                            >
                              <img
                                src={url}
                                alt={`Job photo ${idx + 1}`}
                          ...
                          {deletedPhotos.length > 0 ? (
                            <div className="col-span-2 rounded-md border border-dashed bg-muted/40 p-4 text-sm text-muted-foreground">
                              <div className="font-medium text-foreground">
                                {deletedPhotos[0].message ??
                                  `Photos deleted after ${photoRetention?.retentionDays ?? 90}-day retention period.`}
                              </div>
                              {deletedPhotos[0].deletedAt ? (
                                <div className="mt-1">
                                  Deleted{" "}
                                  {format(
                                    new Date(deletedPhotos[0].deletedAt),
                                    "dd MMM yyyy",
                                  )}
                                </div>
                              ) : null}
                              <div className="mt-1">
                                {deletedPhotos.length} retained photo
                                placeholder
                                {deletedPhotos.length === 1 ? "" : "s"} kept for
                                audit history.
                              </div>
                            </div>
                          ) : null}
