...
export default function WeeklyInvoiceDetail() {
  ...
  const totalInvoiceHours = invoice.lineItems?.reduce((sum, item) => sum + (item.hoursWorked ?? 0), 0) ?? 0;
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        <Button asChild variant="ghost" size="icon">
          <Link href="/weekly-invoices"><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
        <div className="flex-1 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">
              {isWorker ? "My Invoice" : `Invoice: ${invoice.subcontractorName}`}
            </h1>
            <p className="text-muted-foreground">
              {format(new Date(invoice.weekStartDate), 'MMM d')} – {format(new Date(invoice.weekEndDate), 'MMM d, yyyy')}
            </p>
          </div>
          <Badge variant={
            invoice.status === 'paid' ? 'default' :
            invoice.status === 'submitted' ? 'secondary' : 'outline'
              ...
              <Table>
                ...
                <TableBody>
                  {invoice.lineItems?.map((item, i) => (
                    <TableRow key={i}>
                      <TableCell>{item.dispatchDate ? format(new Date(item.dispatchDate), 'MMM d') : '-'}</TableCell>
                      <TableCell>
                        <div className="font-medium">{item.jobTitle}</div>
                        <div className="text-xs text-muted-foreground">{item.jobAddress}</div>
                        {item.jobDescription && (
                          <div className="mt-1 text-xs text-muted-foreground">{item.jobDescription}</div>
                        )}
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="flex justify-end mt-4">
                  <Button
                    variant="secondary"
                    onClick={() => updateInvoice.mutate({ id: invoice.id, data: { notes } })}
                    disabled={updateInvoice.isPending || notes === invoice.notes}
                  >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-medium">{totalInvoiceHours.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
              ...
              <CardFooter className="flex-col gap-3">
                ...
                {!isWorker && invoice.status === 'draft' ? (
                  ...
                  <div className="w-full bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400 p-4 rounded-md flex flex-col items-center justify-center text-center space-y-2">
                    ...
                    <div>
                      <div className="font-bold">Submitted to Xero</div>
                      {invoice.xeroInvoiceId && <div className="text-xs mt-1 opacity-80">ID: {invoice.xeroInvoiceId}</div>}
                    </div>
                  </div>
                ) : null}
              </CardFooter>
