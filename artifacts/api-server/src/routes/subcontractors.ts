...
function serializeSubcontractor(
  ...
  return includeAdminFields
    ? {
        ...base,
        ratePerMetre: sub.ratePerMetre ? Number(sub.ratePerMetre) : null,
      }
    : base;
}
...
router.post("/subcontractors", requireAdmin, async (req, res) => {
  ...
  const [sub] = await db
    ...
    .values({
      ...
      ratePerMetre:
        parsed.data.ratePerMetre != null
          ? String(parsed.data.ratePerMetre)
          : null,
      hourlyRate:
...
router.post(
  "/subcontractors/:id/worker-account",
  requireAdmin,
  async (req, res) => {
    ...
    if (!Number.isInteger(subId) || subId <= 0)
      return res
        .status(400)
        .json({ error: "Invalid employee/subcontractor id" });
    ...
    if (!EMAIL_PATTERN.test(loginEmail))
      return res.status(400).json({ error: "A valid login email is required" });
    ...
    if (loginName.length > 120)
      return res.status(400).json({ error: "Login name is too long" });
    ...
    const [sub] = await db
      ...
      .where(
        and(
          eq(subcontractorsTable.id, subId),
    ...
    if (!sub)
      return res
        .status(404)
        .json({ error: "Employee/subcontractor profile not found" });
    ...
    if (existingByEmail) {
      if (existingByEmail.companyId !== tenantId) {
        return res
          .status(409)
          .json({
            error: "This email is already used by another company account",
          });
      }
      if (existingByEmail.role !== "worker") {
        return res
          .status(409)
          .json({ error: "This email belongs to an admin account" });
      }
      if (
        existingByEmail.subcontractorId &&
        existingByEmail.subcontractorId !== sub.id
      ) {
        return res
          .status(409)
          .json({
            error:
              "This email is already linked to another employee/subcontractor",
          });
      }
    }
    if (
      existingByEmail &&
      existingForSubcontractor &&
      existingByEmail.id !== existingForSubcontractor.id
    ) {
      return res
        .status(409)
        .json({
          error:
            "This employee/subcontractor already has a different login account",
        });
    }
    ...
    const account = await db.transaction(async (tx) => {
      ...
      const [savedAccount] = existingAccount
        ? await tx
            ...
            .returning()
        : await tx.insert(appUsersTable).values(userValues).returning();
      ...
      if (sub.email !== loginEmail) {
        await tx
          ...
          .where(
            and(
              eq(subcontractorsTable.id, sub.id),
              eq(subcontractorsTable.companyId, tenantId),
            ),
          );
      }
...
router.get("/subcontractors/:id", async (req, res) => {
  const parsed = GetSubcontractorParams.safeParse({
  ...
  if (!canAccessSubcontractor(req, parsed.data.id)) {
    return res
      .status(403)
      .json({
        error: "You can only view your own employee/subcontractor profile",
      });
  }
  const [sub] = await db
    .select()
    ...
    .where(
      and(
        eq(subcontractorsTable.id, parsed.data.id),
        eq(subcontractorsTable.companyId, companyId(req)),
      ),
    );
...
router.patch("/subcontractors/:id", requireAdmin, async (req, res) => {
  const params = UpdateSubcontractorParams.safeParse({
    id: Number(req.params.id),
  });
  ...
  if (body.data.ratePerMetre !== undefined)
    updates.ratePerMetre = String(body.data.ratePerMetre);
  ...
  const [sub] = await db
    ...
    .where(
      and(
        eq(subcontractorsTable.id, params.data.id),
        eq(subcontractorsTable.companyId, companyId(req)),
      ),
    )
    .returning();
